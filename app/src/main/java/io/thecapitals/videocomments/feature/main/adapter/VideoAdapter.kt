...
class VideoAdapter(private val viewModel: VideoListViewModel) : RecyclerView.Adapter<BaseViewHolder<*>>() {

    override fun onBindViewHolder(holder: BaseViewHolder<*>, position: Int) {
        val item: ItemVideoBinding = holder.binding as ItemVideoBinding
        val viewModel: VideoItemViewModel = viewModel.provideViewModelItem(position)
        item.viewModel = viewModel
        item.executePendingBindings()
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseViewHolder<ItemVideoBinding> {
        return BaseViewHolder(DataBindingUtil.inflate(LayoutInflater.from(parent.context), R.layout.item_video, parent, false))
    }
